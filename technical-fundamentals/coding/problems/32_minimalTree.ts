...
export default function minimalTree<T>(
  ...
  const value = sortedArray[midIndex];

  const left = minimalTree(sortedArray.slice(0, midIndex));
  const right = minimalTree(
    sortedArray.slice(midIndex + 1, sortedArray.length)
  );
